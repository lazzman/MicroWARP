...
# 手工操作与计划重启使用相同的默认恢复确认窗口；手工路径不再因默认值不同
# 而比计划重启多等待一轮。显式设置时仍允许按部署需要覆盖。
MANAGEMENT_ACTION_PROBE_TIMEOUT="${MANAGEMENT_ACTION_PROBE_TIMEOUT:-90}"
# 优雅管理操作检测到仍有连接后，以短周期复查；这只影响手工停用/优雅重连，
# 不改变计划滚动重启的 60 秒延后队列节奏。
MANAGEMENT_IDLE_CHECK_INTERVAL="${MANAGEMENT_IDLE_CHECK_INTERVAL:-3}"
# 管理操作和计划滚动重启共享的 WARP 就绪复查节奏。
RESTART_RECOVERY_PROBE_INTERVAL="${RESTART_RECOVERY_PROBE_INTERVAL:-3}"
[[ "$MANAGEMENT_ACTION_PROBE_TIMEOUT" =~ ^[0-9]+$ ]] && [ "$MANAGEMENT_ACTION_PROBE_TIMEOUT" -gt 0 ] || MANAGEMENT_ACTION_PROBE_TIMEOUT=90
[[ "$MANAGEMENT_IDLE_CHECK_INTERVAL" =~ ^[0-9]+$ ]] && [ "$MANAGEMENT_IDLE_CHECK_INTERVAL" -gt 0 ] || MANAGEMENT_IDLE_CHECK_INTERVAL=3
[[ "$RESTART_RECOVERY_PROBE_INTERVAL" =~ ^[0-9]+$ ]] && [ "$RESTART_RECOVERY_PROBE_INTERVAL" -gt 0 ] || RESTART_RECOVERY_PROBE_INTERVAL=3
...
# auto：按当前实例总数的 1/5 并行滚动；正整数可显式指定请求并发数量。
ROTATE_RESTART_CONCURRENCY="${ROTATE_RESTART_CONCURRENCY:-auto}"
# 无论 auto 或显式请求的并行数为何，都不能超过该硬上限，避免大规模实例
# 同时摘流、重启和探测造成控制面及上游瞬时拥塞。
ROTATE_RESTART_MAX_CONCURRENCY="${ROTATE_RESTART_MAX_CONCURRENCY:-10}"
[[ "$ROTATE_RESTART_MAX_CONCURRENCY" =~ ^[0-9]+$ ]] && [ "$ROTATE_RESTART_MAX_CONCURRENCY" -gt 0 ] || ROTATE_RESTART_MAX_CONCURRENCY=10
...
export_control_environment() {
    ...
    export LB_ACCESS_LOG_HEADER_MAX_CHARS MANAGEMENT_UI_ENABLED MANAGEMENT_ACTION_PROBE_TIMEOUT MANAGEMENT_IDLE_CHECK_INTERVAL RESTART_RECOVERY_PROBE_INTERVAL INTERNAL_PROXY_PORT
    ...
    export BACKEND_POOL_LOCK_TIMEOUT BACKEND_POOL_BATCH_WINDOW_MS BACKEND_POOL_STALE_LOCK_TIMEOUT BACKEND_POOL_OPERATION_RETRY_LIMIT BACKEND_POOL_OPERATION_RETRY_MAX_DELAY
    export ROTATE_RESTART_INTERVAL ROTATE_RESTART_PROBE_TIMEOUT ROTATE_RESTART_RETRIES
    export ROTATE_RESTART_CONCURRENCY ROTATE_RESTART_MAX_CONCURRENCY ROTATE_RESTART_DEFERRED_CHECK_INTERVAL ROTATE_RESTART_HISTORY_LIMIT RESOLVE_PREFERENCE UPSTREAM_SOCKS5
