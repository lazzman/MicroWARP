...
def restart_schedule_concurrency(scope_count: int) -> int:
    ...
    maximum = max(1, positive_int(os.environ.get("ROTATE_RESTART_MAX_CONCURRENCY", "10"), 10))
    configured = os.environ.get("ROTATE_RESTART_CONCURRENCY", "auto").strip().lower()
    if configured.isdigit() and int(configured) >= 1:
        return min(total, maximum, int(configured))
    return min(total, maximum, max(1, total // 5))
...
def restart_schedule_snapshot(now: Optional[int] = None) -> Dict[str, object]:
    ...
    return {
        ...
        "policy": {
            ...
            "max_concurrency": max(
                1, positive_int(os.environ.get("ROTATE_RESTART_MAX_CONCURRENCY", "10"), 10)
            ),
...
# 管理操作状态会在阶段切换时原子覆盖。这些字段保留每个关键边界的时间，既供页面
# 解释全链路耗时，也让“超时后恢复”不再丢失最关键的排障上下文。
MANAGEMENT_OPERATION_MILESTONE_FIELDS = (
    "idle_wait_started_at", "idle_confirmed_at", "backend_drained_at",
    "backend_restored_at", "instance_stop_started_at", "instance_stopped_at",
    "instance_start_started_at", "instance_started_at", "warp_probe_started_at",
    "warp_ready_at", "pool_rejoined_at", "probe_timed_out_at", "health_recovered_at",
)
...
def enrich_management_operation(operation: Dict[str, str]) -> Dict[str, object]:
    ...
    # 将阶段时间标准化为整数，避免前端把状态文件中的字符串误作日期或进行拼接。
    for milestone in MANAGEMENT_OPERATION_MILESTONE_FIELDS:
        milestone_at = timestamp(milestone)
        if milestone_at:
            result[milestone] = milestone_at
...
def management_status_snapshot() -> Dict[str, object]:
    ...
    return {
        ...
        "management": {
            ...
            # 保留旧字段以兼容已有面板，同时明确它是手工优雅操作而非计划队列的
            # 复查周期。计划队列周期继续由 restart_schedule.policy 提供。
            "deferred_check_interval": os.environ.get("MANAGEMENT_IDLE_CHECK_INTERVAL", "3"),
            "deferred_check_interval_seconds": max(
                1, duration_seconds(os.environ.get("MANAGEMENT_IDLE_CHECK_INTERVAL", "3"), 3)
            ),
            "idle_check_interval": os.environ.get("MANAGEMENT_IDLE_CHECK_INTERVAL", "3"),
            "idle_check_interval_seconds": max(
                1, duration_seconds(os.environ.get("MANAGEMENT_IDLE_CHECK_INTERVAL", "3"), 3)
            ),
            "recovery_probe_interval": os.environ.get("RESTART_RECOVERY_PROBE_INTERVAL", "3"),
            "recovery_probe_interval_seconds": max(
                1, duration_seconds(os.environ.get("RESTART_RECOVERY_PROBE_INTERVAL", "3"), 3)
            ),
            "action_probe_timeout": max(
                1, positive_int(os.environ.get("MANAGEMENT_ACTION_PROBE_TIMEOUT", "90"), 90)
            ),
        },
    }
...
function phaseLabel(phase){return {received:'已接收请求','wait-idle':'等待自然空闲','backend-drain':'后端池摘流','backend-restore':'恢复后端池流量','pool-rejoin':'确认重新入池',reconnect:'重建 WARP 连接','instance-start':'启动实例','instance-stop':'停止实例','warp-probe':'WARP 健康探测',recovered:'健康守护恢复',controller:'管理控制器',completed:'完成',unknown:'未知阶段'}[phase]||phase||'—'}
