...
def restart_schedule_run_summary(run_id: str) -> Optional[Dict[str, object]]:
    ...
    numeric_keys = {
        ...
        "skipped", "deferred", "max_queued", "max_deferred", "avg_deferred_wait_seconds",
        "deferred_check_interval_seconds",
    }
...
def restart_schedule_queue_snapshot(queue_name: str) -> Optional[Dict[str, object]]:
    """按实例状态文件构建当前就绪/延后队列，避免仅显示过期的汇总计数。"""
    if queue_name not in {"ready", "deferred"}:
        return None
    entries: List[Dict[str, object]] = []
    for instance_id in runtime_instance_ids():
        ...
        for key in ("queue_position", "queue_total", "active_connections", "started_at", "queue_entered_at", "deferred_at", "next_check_at", "updated_at"):
            if key in values:
...
def update_restart_schedule_pause(action: str) -> Tuple[bool, int, str, Dict[str, object]]:
    """通过运行时标记暂停或恢复后续轮次，不改写用户的容器环境配置。"""
...
def scheduled_restart_operation_snapshot(runtime: str) -> Dict[str, object]:
    """读取定时重启的等待或延后状态；实际执行阶段由 rotation.state 覆盖。"""
...
function scheduleStatePresentation(schedule){const state=schedule&&schedule.status||'waiting';if(state==='disabled')return {label:'未启用',kind:'off'};if(state==='unavailable')return {label:'仅多实例',kind:'off'};if(state==='paused')return {label:'已暂停',kind:'off'};if(state==='running')return {label:'执行中',kind:'wait'};if(state==='starting')return {label:'正在初始化',kind:'wait'};return {label:'已启用',kind:'ok'}}
function restartScheduleMessage(schedule){const state=schedule&&schedule.status||'waiting';if(state==='disabled')return '当前滚动重启配置未启用。请调整 ROTATE_RESTART_ENABLED 后重建容器。';if(state==='unavailable')return '定时滚动重启仅适用于多实例部署；当前实例数量不足，无需创建执行计划。';if(state==='paused')return '后续定时轮次已暂停；恢复后会按当前间隔重新计算下一次执行时间。';if(state==='running')return schedule.pause_requested?'本轮滚动重启仍在执行，后续定时轮次已暂停。':'当前正在按空闲优先策略滚动重启，繁忙实例会保持服务并按配置间隔复查，直到自然空闲。';return '计划按空闲优先的连续队列执行：空闲实例立即重启；仍有活跃连接的实例保持服务并按配置间隔复查，直到自然空闲。实际执行时间由容器内调度器决定，页面按浏览器本地时区显示。'}
function renderRestartSchedule(status){const schedule=status.restart_schedule||{},presentation=scheduleStatePresentation(schedule),interval=Number(schedule.interval_seconds||0),policy=schedule.policy||{},current=schedule.current||{},result=schedule.last_result||null,state=String(schedule.status||'waiting'),ready=Number(current.queued||0),deferred=Number(current.deferred||0),nextDeferred=Number(current.next_deferred_check_at||0);const badge=el('restart-schedule-state');badge.className='badge '+presentation.kind;badge.textContent=presentation.label;el('restart-schedule-note').textContent=state==='running'?'本轮执行期间自动提升刷新频率':state==='paused'?'暂停仅影响后续定时轮次':state==='disabled'||state==='unavailable'?'由容器环境变量控制':'计划由调度器持续维护';let summary='每 '+duration(interval)+' 执行一次';if(state==='disabled')summary='未配置定时滚动重启';else if(state==='unavailable')summary='单实例部署无需滚动重启计划';else if(state==='paused')summary='后续定时滚动重启已暂停';else if(state==='running'){const total=Number(current.total||0),completed=Number(current.completed||0);summary='正在滚动重启'+(total?'：已处理 '+completed+' / '+total+' 个实例':'');}el('restart-schedule-summary').textContent=summary;const deferredInterval=duration(Number(policy.deferred_check_interval_seconds||60)),policyText=(Number(schedule.scope_count||0))+' 个实例 · 空闲优先 · 最大并行 '+Number(policy.concurrency||1)+' · 繁忙实例每 '+deferredInterval+' 复查';el('restart-schedule-policy').textContent=policyText;el('restart-ready-count').textContent=String(ready);el('restart-deferred-count').textContent=String(deferred);el('restart-ready-queue').disabled=ready===0;el('restart-deferred-queue').disabled=deferred===0;el('restart-ready-queue').title=ready?'查看 '+ready+' 个就绪队列实例':'当前无就绪队列实例';el('restart-deferred-queue').title=deferred?'查看 '+deferred+' 个延后队列实例':'当前无延后队列实例';el('restart-deferred-next').textContent=deferred?(nextDeferred?'下次复查 '+timestampText(nextDeferred)+'（约 '+duration(Math.max(0,nextDeferred-Math.floor(Date.now()/1000)))+' 后）':'等待调度器记录下次复查时间'):'当前无待复查实例';const next=Number(schedule.next_run_at||0);if(state==='waiting'&&next){const remaining=Math.max(0,next-Math.floor(Date.now()/1000));el('restart-schedule-next').textContent=timestampText(next);el('restart-schedule-countdown').textContent='约 '+duration(remaining)+' 后';}else if(state==='running'){el('restart-schedule-next').textContent=nextDeferred?'延后队列复查':'本轮执行中';el('restart-schedule-countdown').textContent=nextDeferred?'下次复查 '+timestampText(nextDeferred)+'（约 '+duration(Math.max(0,nextDeferred-Math.floor(Date.now()/1000)))+' 后） · 就绪 '+ready+' · 延后 '+deferred:'已处理 '+Number(current.completed||0)+' · 就绪 '+ready+' · 延后 '+deferred+' · 成功 '+Number(current.succeeded||0)+' · 失败 '+Number(current.failed||0);}else{el('restart-schedule-next').textContent='—';el('restart-schedule-countdown').textContent=state==='disabled'?'设置后可启用':state==='unavailable'?'需要至少 2 个实例':state==='paused'?'恢复后重新计算':'等待调度器初始化';}const lastAt=Number(schedule.last_run_at||0);el('restart-schedule-last').textContent=lastAt?timestampText(lastAt):'—';el('restart-schedule-result').textContent=scheduleResultText(result);const failures=Number(result&&result.failed||0),historyCount=Number((schedule.history||[]).length),failureButton=el('restart-schedule-failures'),historyButton=el('restart-schedule-history'),actionBar=el('restart-schedule-actions'),hasFailureDetails=failures>0&&Boolean(result&&result.run_id);failureButton.hidden=!hasFailureDetails;failureButton.disabled=!hasFailureDetails;el('restart-schedule-failure-count').textContent=String(failures);historyButton.hidden=historyCount===0;historyButton.disabled=historyCount===0;el('restart-schedule-history-count').textContent=String(historyCount);actionBar.hidden=!hasFailureDetails&&historyCount===0;el('restart-schedule-dialog-message').textContent=restartScheduleMessage(schedule);const details=el('restart-schedule-details');details.replaceChildren();const queueText='就绪队列 '+ready+' · 延后队列 '+deferred+(deferred?(nextDeferred?' · 下次复查 '+timestampText(nextDeferred):' · 等待下次复查时间'):' · 当前无待复查实例'),rows=[['执行周期','每 '+duration(interval)+' 运行一次（配置值：'+text(schedule.interval)+'）'],['范围与策略',policyText],['队列状态',queueText],['下次执行',state==='waiting'&&next?timestampText(next)+'（约 '+duration(Math.max(0,next-Math.floor(Date.now()/1000)))+' 后）':state==='running'?'当前轮次执行中':state==='paused'?'计划已暂停':state==='unavailable'?'仅多实例可用':'—'],['最近执行',lastAt?timestampText(lastAt)+' · '+scheduleResultText(result)+(failures?' · 可查看 '+failures+' 条失败明细':''):'尚无执行记录']];for(const [label,value] of rows){const row=document.createElement('div'),title=document.createElement('strong');row.className='restart-schedule-detail';title.textContent=label;row.append(title,document.createTextNode(value));details.append(row)}const toggle=el('restart-schedule-toggle'),canToggle=Boolean(schedule.config_active&&schedule.scheduler_available);toggle.disabled=!canToggle;toggle.textContent=schedule.paused?'恢复计划':'暂停计划';toggle.title=canToggle?'':schedule.config_active?'定时滚动重启仅适用于多实例部署':'当前 ROTATE_RESTART_ENABLED 配置未启用';}
...
async function openRestartQueue(kind,trigger){restartDetailsTrigger=trigger||document.activeElement;el('restart-queue-title').textContent=kind==='ready'?'就绪队列实例':'延后队列实例';el('restart-queue-note').textContent='正在读取当前队列快照。';const dialog=el('restart-queue-dialog');if(!dialog.open)dialog.showModal();restartDetailEmpty(el('restart-queue-list'),'正在读取队列实例…');try{const data=await requestJson(base+'restart-schedule/runs/current/queue/'+kind),records=data.instances||[];el('restart-queue-note').textContent=kind==='ready'?'空闲实例会按并行额度连续处理。':'繁忙实例保持服务；连接自然归零后转入就绪队列。';const list=el('restart-queue-list');if(!records.length){restartDetailEmpty(list,'当前无'+(kind==='ready'?'就绪':'延后')+'队列实例');return}list.replaceChildren();for(const record of records){const card=document.createElement('article'),title=document.createElement('strong'),detail=document.createElement('p'),meta=document.createElement('small');card.className='restart-record';title.textContent='实例 '+text(record.instance_id);detail.textContent=kind==='ready'?'队列位置 '+text(record.queue_position)+' / '+text(record.queue_total)+' · '+text(record.message):'活跃连接 '+Number(record.active_connections||0)+' · '+text(record.message);meta.textContent=kind==='ready'?'入队 '+timestampText(record.queue_entered_at):'延后于 '+timestampText(record.deferred_at)+' · 下次复查 '+timestampText(record.next_check_at);card.append(title,detail,meta);list.append(card)}}catch(error){el('restart-queue-note').textContent='读取失败：'+error.message;restartDetailEmpty(el('restart-queue-list'),'无法读取队列实例') }}
...
function openRestartSchedule(trigger){restartScheduleTrigger=trigger||document.activeElement;const dialog=el('restart-schedule-dialog');if(lastStatus)renderRestartSchedule(lastStatus);if(!dialog.open)dialog.showModal();requestAnimationFrame(()=>{const toggle=el('restart-schedule-toggle');(toggle.disabled?el('restart-schedule-close'):toggle).focus()})}
...
function statusLabel(status){return {queued:'已排队',deferred:'等待自然空闲',claiming:'正在摘流复核',running:'执行中',draining:'连接排空中',restarting:'正在重启实例',probing:'WARP 健康探测中',reconnecting:'正在重建 WARP',starting:'正在启动',stopping:'正在停止',success:'已完成',recovered:'超时后已恢复',failed:'执行失败',partial:'部分完成',cancelled:'已取消'}[status]||status||'等待状态更新'}
...
function hasActiveTasks(){return Boolean((lastStatus&&lastStatus.resize_operation_running)||(lastStatus&&lastStatus.restart_schedule&&lastStatus.restart_schedule.status==='running')||[...pendingInstances].length||(lastStatus&&lastStatus.instances||[]).some(item=>isBusy(item)))}
...
def handle_management_request(client: socket.socket, lines: List[str], body: bytes = b"") -> bool:
    ...
    if method == "GET" and path.startswith(restart_schedule_api_prefix + "/runs/"):
        ...
        parts = suffix.split("/")
        if len(parts) == 3 and parts[1] == "queue" and parts[2] in {"ready", "deferred"} and parts[0] == "current":
    ...
    if method == "POST" and path == MANAGEMENT_BASE_PATH + "/api/v1/restart-schedule":
        ...
        accepted, status_code, message, schedule = update_restart_schedule_pause(action)
        status = (
            "200 OK"
            if accepted
            else "404 Not Found"
