...
update_backend() {
    local id="$1" target_state="$2" lock pending request temporary_request started elapsed
    [[ "$id" =~ ^[0-9]+$ ]] || return 2
    ...
    while true; do
        ...
        # 本请求已经被当前提交者合并，无需再等待或重复重建后端池。
        [[ ! -e "$request" ]] && return 0
        recover_stale_backend_lock "$lock"
        elapsed=$(( $(date +%s) - started ))
        # 即使外部管理动作长时间持锁，请求也已落盘，下一位提交者会继续处理；
        # 不能再像旧版那样 5 秒后直接丢弃 up/down 更新并制造告警风暴。
        [[ "$BACKEND_POOL_LOCK_TIMEOUT" -eq 0 || "$elapsed" -lt "$BACKEND_POOL_LOCK_TIMEOUT" ]] || return 0
